...
import { ChevronRight, ExternalLink, Check, Plus, Minus, Users, ArrowRight } from "lucide-react"
...
import { tariffs, banks } from "@/lib/mock-data"
...
export default function TariffPage({ params }: TariffPageProps) {
  ...
  const similarTariffs = tariffs
    .filter((t) => t.id !== tariff.id && (Math.abs(t.price - tariff.price) < 1000 || t.bankId === tariff.bankId))
    .slice(0, 4)
      ...
      <main className="flex-1 bg-background-secondary">
        <div className="container-custom py-8 md:py-12">
          ...
          <nav className="flex items-center gap-2 text-sm text-foreground-secondary mb-6 flex-wrap">
            ...
            <ChevronRight className="h-4 w-4" />
            <Link href={`/banks/${bank.id}`} className="hover:text-primary transition-colors">
              {bank.name}
            </Link>
            <ChevronRight className="h-4 w-4" />
          ...
          <div className="bg-background rounded-xl border border-border p-6 md:p-8 mb-8">
            <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
              <div className="flex items-start gap-4 md:gap-6">
                ...
                <div>
                  <div className="flex items-center gap-3 mb-2 flex-wrap">
                    <h1 className="text-2xl md:text-3xl font-bold text-foreground">{tariff.name}</h1>
                  ...
                  <div className="mt-4">
                    <p className="text-sm text-foreground-muted">Стоимость обслуживания</p>
                    <p className="text-3xl md:text-4xl font-bold text-foreground">{tariff.priceLabel}</p>
                  </div>
                </div>
              </div>
              <div className="flex flex-col sm:flex-row gap-3 lg:flex-col xl:flex-row">
                ...
                <Button size="lg" variant={inComparison ? "secondary" : "outline"} onClick={handleCompareClick}>
                  {inComparison ? (
                    <>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-6">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      <tbody className="divide-y divide-border">
                        <tr>
                          <td className="py-3 text-foreground-secondary">Стоимость обслуживания</td>
                          <td className="py-3 font-medium text-foreground text-right">{tariff.priceLabel}</td>
                        </tr>
                        <tr>
                          <td className="py-3 text-foreground-secondary">Лимит операций</td>
                          <td className="py-3 font-medium text-foreground text-right">
                            {tariff.operationsLimit} платежей/мес
                          </td>
                        </tr>
                        <tr>
                          <td className="py-3 text-foreground-secondary">Бесплатных переводов</td>
                          <td className="py-3 font-medium text-foreground text-right">{tariff.freeTransfers}</td>
                        </tr>
                        <tr>
                          <td className="py-3 text-foreground-secondary">Комиссия за перевод</td>
                          <td className="py-3 font-medium text-foreground text-right">{tariff.transferCommission}</td>
                        </tr>
                        <tr>
                          <td className="py-3 text-foreground-secondary">Снятие наличных</td>
                          <td className="py-3 font-medium text-foreground text-right">
              ...
              <Card>
                ...
                <CardContent>
                  <p className="text-foreground-secondary leading-relaxed">{tariff.targetAudience}</p>
                  <p className="text-foreground-secondary leading-relaxed mt-3">{tariff.description}</p>
                </CardContent>
              </Card>
            ...
            <div className="space-y-6">
              ...
              <Card className="bg-primary text-primary-foreground">
                <CardContent className="pt-6">
                  <h3 className="text-xl font-semibold mb-3">Готовы открыть счёт?</h3>
                  <p className="text-primary-foreground/80 mb-6">
                    Перейдите на сайт банка и оформите РКО онлайн за несколько минут
                  </p>
              ...
              <Card>
                <CardContent className="pt-6">
                  ...
                  <p className="text-sm text-foreground-secondary mb-4">
                    Воспользуйтесь умным подбором, чтобы найти оптимальный тариф под ваш бизнес
                  </p>
          ...
          {similarTariffs.length > 0 && (
            <section className="mt-12">
              <h2 className="text-2xl font-bold text-foreground mb-6">Похожие тарифы</h2>
