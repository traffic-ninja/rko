...
const comparisonRows = [
	...
	{ key: "price_label", label: "Стоимость обслуживания" },
	{ key: "free_transfers", label: "Бесплатных платежей" },
	{ key: "operations_limit", label: "Лимит операций" },
	{ key: "transfer_commission", label: "Комиссия за перевод" },
	{ key: "cash_withdrawal_commission", label: "Снятие наличных" },
	{ key: "target_audience", label: "Для кого подходит" },
] as const;
	...
	const bestTariff =
		comparedTariffs.length >= 2
			? comparedTariffs.reduce((best, current) => {
					const bestScore = best.free_transfers * 10 - best.price;
					const currentScore = current.free_transfers * 10 - current.price;
					return currentScore > bestScore ? current : best;
				})
			: null;

	const filteredTariffs = initialTariffs.filter(
		(t) =>
			...
			case "bank_name":
				return (
					<div className="flex items-center gap-2">
						...
						/>{" "}
						<span>{tariff.bank_name}</span>
					</div>
				);
			case "price_label":
				return <span className="font-semibold">{tariff.price_label}</span>;
			case "free_transfers":
											...
											<div className="flex items-start gap-4">
												...
												<div>
													...
													<p className="text-foreground-secondary">
														...
														{bestTariff.price_label} вы получаете{" "}
														{bestTariff.free_transfers} бесплатных платежей.
													</p>
												</div>
											</div>
											<Button
												...
												onClick={(e) => e.preventDefault()} // Placeholder, replace with actual link logic if needed
											>
												Открыть РКО в {bestTariff.bank_name}
											</Button>{" "}
										</div>
									</CardContent>
								</Card>
