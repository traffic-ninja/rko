...
import { Plus, X, Trash2, ExternalLink, Trophy, Search } from "lucide-react"
...
export default function ComparisonPage() {
  const { comparedTariffs, addToComparison, removeFromComparison, clearComparison } = useComparison()
      ...
      case "bankName":
        return (
          <div className="flex items-center gap-2">
            <img src={tariff.bankLogo || "/placeholder.svg"} alt="" className="h-6 w-6 rounded" />
      ...
      <main className="flex-1 bg-background-secondary">
        <div className="container-custom py-8 md:py-12">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
            <div>
              <h1 className="text-3xl md:text-4xl font-bold text-foreground mb-2">Сравнение тарифов РКО</h1>
              <p className="text-foreground-secondary">Сравните условия выбранных тарифов для принятия решения</p>
            </div>
          ...
          {comparedTariffs.length === 0 ? (
            ...
            <Card className="text-center py-16">
              <CardContent>
                ...
                <h2 className="text-xl font-semibold text-foreground mb-2">Вы ещё не добавили тарифы для сравнения</h2>
                <p className="text-foreground-secondary mb-8 max-w-md mx-auto">
                  Выберите тарифы на страницах банков или воспользуйтесь умным подбором
                </p>
                            ...
                            filteredTariffs.map((tariff) => (
                              ...
                              >
                                ...
                                <div className="flex-1 min-w-0">
                                  <p className="font-medium text-foreground truncate">{tariff.name}</p>
                                  <p className="text-sm text-foreground-secondary">{tariff.bankName}</p>
                                </div>
                                <span className="text-sm font-medium text-primary shrink-0">{tariff.priceLabel}</span>
                              </button>
                            ))
                          ) : (
                            <p className="text-center text-foreground-secondary py-8">Тарифы не найдены</p>
                          )}
                        </div>
                      </div>
                    </DialogContent>
              ...
              {comparedTariffs.length === 1 && (
                <Card className="mb-6 bg-info/10 border-info/20">
                  <CardContent className="py-4">
                    <p className="text-sm text-info">Добавьте ещё минимум 1 тариф для сравнения</p>
                  </CardContent>
                </Card>
              )}
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-border">
                        ...
                        {comparedTariffs.map((tariff) => (
                          <th key={tariff.id} className="p-4 min-w-[200px] bg-background">
                            <div className="relative">
                              <button
                    ...
                    <tbody>
                      {comparisonRows.map((row, index) => (
                        ...
                        >
                          <td className="p-4 text-sm font-medium text-foreground-secondary">{row.label}</td>
                          {comparedTariffs.map((tariff) => (
                            <td key={tariff.id} className="p-4 text-sm text-foreground">
                              {getValue(tariff, row.key)}
                            </td>
                          ))}
                          {comparedTariffs.length < 3 && <td className="p-4"></td>}
                        </tr>
                      ))}
                      {/* Actions Row */}
                  ...
                  <CardContent className="py-6">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          <h3 className="text-lg font-semibold text-foreground">Рекомендуем: {bestTariff.name}</h3>
                          <p className="text-foreground-secondary">
                            Оптимальное соотношение цены и количества бесплатных операций для вашего бизнеса. При
                            стоимости {bestTariff.priceLabel} вы получаете {bestTariff.freeTransfers} бесплатных
                            платежей.
                          </p>
                        </div>
                      </div>
