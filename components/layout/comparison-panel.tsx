...
export function ComparisonPanel() {
  const { comparedTariffs, removeFromComparison, clearComparison } = useComparison()
          ...
          <div className="flex items-center gap-4 flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 overflow-x-auto">
              {comparedTariffs.map((tariff) => (
                <div key={tariff.id} className="flex items-center gap-2 bg-muted rounded-md px-3 py-1.5 shrink-0">
                  <img src={tariff.bankLogo || "/placeholder.svg"} alt={tariff.bankName} className="h-5 w-5 rounded" />
                  <span className="text-sm font-medium text-foreground truncate max-w-[120px]">{tariff.name}</span>
                  <button
