...
import { TariffJsonLd, BreadcrumbJsonLd } from "@/components/structured-data";
import type { Tariff, Bank } from "@/lib/supabase/types";

interface TariffPageProps {
  params: Promise<{ tariffId: string }>;
}

export async function generateMetadata({ params }: TariffPageProps): Promise<Metadata> {
...
export default async function TariffPage({ params }: TariffPageProps) {
  ...
  const { data: similarTariffsData, error: similarTariffsError } = await supabase
    ...
    .select("*")
    .neq("id", tariff.id)
    .or(`price.gte.${tariff.price - 1000},price.lte.${tariff.price + 1000},bank_id.eq.${tariff.bank_id}`)
    .limit(4);
