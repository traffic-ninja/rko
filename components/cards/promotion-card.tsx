...
export function PromotionCard({ promotion }: PromotionCardProps) {
  const isExpiringSoon = promotion.expiresAt
    ? new Date(promotion.expiresAt).getTime() - Date.now() < 7 * 24 * 60 * 60 * 1000
    : false
      ...
      <CardContent className="p-6">
        <div className="flex items-start gap-4 mb-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-foreground-secondary">{promotion.bankName}</p>
          </div>
        </div>

        <p className="text-sm text-foreground-secondary mb-4">{promotion.description}</p>
        ...
        {promotion.expiresAt && (
          ...
          >
            ...
            <span>
              До {new Date(promotion.expiresAt).toLocaleDateString("ru-RU", { day: "numeric", month: "long" })}
            </span>
          </div>
        )}
